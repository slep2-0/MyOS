...
build/apc.o: kernel/core/me/apc.c
	mkdir -p build
	$(CC) $(CFLAGS) $< -o $@ >> log.txt 2>&1
	
build/synchinit.o: kernel/core/ms/synchinit.c
	mkdir -p build
	$(CC) $(CFLAGS) $< -o $@ >> log.txt 2>&1

build/wait.o: kernel/core/ms/wait.c
	mkdir -p build
	$(CC) $(CFLAGS) $< -o $@ >> log.txt 2>&1
...
# Link kernel
build/kernel.elf: build/kernel_entry.o build/assert.o build/kernel.o build/idt.o build/isr.o build/handlers.o build/pfn.o build/attach.o build/pushlock.o build/synchinit.o build/wait.o build/instruction.o build/section.o build/setup.o build/handler.o build/exception.o \
                      build/hypermap.o build/bugcheck.o build/map.o build/ahci.o build/block.o build/ob.o build/apc.o build/psmgr.o build/pswork.o build/handle.o build/cid.o build/syscallEntryAsm.o build/systemcalls.o build/probe.o build/raise.o \
