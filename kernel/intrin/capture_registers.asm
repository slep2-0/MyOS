...
read_context_frame:
    ...
    mov [rdi + 0x40], rbp
    mov [rdi + 0x48], rdi ; Note: rdi is also input pointer (the argument) FIXME TODO since rdi is the input pointer, we are copying only like 4 registers that were before, and now we are just copying the struct registers themselves.
    mov [rdi + 0x50], rsi
    mov [rdi + 0x58], rdx
    mov [rdi + 0x60], rcx
    mov [rdi + 0x68], rbx
