...
Purpose:
    ...
    This translation unit contains the implementation of the object manager creation and handle functions.
...

{
    ...
    // Initalize object namespace.
    MTSTATUS Status = ObpInitializeDirectoryType();
    if (MT_FAILURE(Status)) return Status;

    Status = ObpInitializeSymbolicLinkType();
    if (MT_FAILURE(Status)) return Status;

    Status = ObpCreateRootDirectory();
    if (MT_FAILURE(Status)) return Status;

    Status = ObpCreateAchtungNamedObjectsDirectory();
    if (MT_FAILURE(Status)) return Status;

    Status = ObpCreateDeviceDirectory();
    return Status;
}
...

{
    ...
    // If the object has NameInfo, then call deletion upon it
    if (Header->NameInfo) {
