...
static void map_physical_range(uint64_t phys, size_t length, uint32_t flags) {
	uint64_t start = phys & PAGE_MASK;
	uint64_t end = ((phys + length) + PAGE_SIZE_4K- 1) & PAGE_MASK;
	for (uint64_t p = start; p < end; p += PAGE_SIZE_4K) {
		void* v = MtTranslatePhysicalMemoryToVirtualOffset(p);
		map_page(v, p, flags);
	}
}

static void* FindACPIHeader(XSDT* xsdt, const char* headerSignature) {
	...
	for (uint32_t i = 0; i < entries; ++i) {
		...
		ACPI_SDT_HEADER* hdr = (ACPI_SDT_HEADER*)MtTranslatePhysicalMemoryToVirtualOffset(headerPhys);
...
void MtACPIRebootComputer(void) {
	...
	case 0: // Memory mapped (MMIO)
		...
		map_page((void*)phys, (uintptr_t)phys, PAGE_PRESENT | PAGE_RW | PAGE_PCD);
...
MTSTATUS InitializeACPI(void) {
	...
	if (!rsdpPhys) return (MTSTATUS)0xDEAD;
	map_physical_range(rsdpPhys, sizeof(RSDP_Descriptor), PAGE_PRESENT | PAGE_RW | PAGE_PCD);
	RSDP_Descriptor* rsdp = (RSDP_Descriptor*)MtTranslatePhysicalMemoryToVirtualOffset(rsdpPhys);
	...
	XSDT* xsdt = (XSDT*)MtTranslatePhysicalMemoryToVirtualOffset(xsdtPhys);
	...
	ACPI_SDT_HEADER* facp = (ACPI_SDT_HEADER*)FindACPIHeader(xsdt, "FACP");
	...
	// Find MADT
	ACPI_SDT_HEADER* madtACPI = (ACPI_SDT_HEADER*)FindACPIHeader(xsdt, "APIC");
