...
/*++
    ...
    Return Values:
        ...
        Highest Usable Address in physical memory (total amount of usable RAM)

        DO NOTE: The function will return the max address that is usable in the system, which might leave MMIO holes above, these will NOT be added into the PFN DB.
...

{
    ...
    MiReservePhysRange(pfn_region_phys, neededPages * VirtualPageSize);
    ...
    for (size_t i = 0; i < entryCount; i++) {
        ...
        for (uint64_t p = 0; p < regionPages; p++) {
            ...
            // UEFI Memory maps arent sorted in order, we could get address 0x100000 at one iteration, and address 0x2000 at the next iteration.
            // We guard it with a simple if statement.
