...
void frame_bitmap_init(void) {
    ...
    for (size_t i = 0; i < entry_count; ++i) {
        ...
        if (classify(desc->Type) && (desc->NumberOfPages * FRAME_SIZE) >= bitmap_size) {
            ...
            frame_bitmap = (uint8_t*)desc->PhysicalStart;
            break;
        }
    ...
    for (size_t i = 0; i < bitmap_pages; i++) {
        set_frame(((uintptr_t)frame_bitmap / FRAME_SIZE) + i);
    }
    ...
    for (size_t i = 0; i < entry_count; ++i) {
        if (desc->Type == EfiConventionalMemory) {
            ...
            for (uint64_t p = 0; p < pages; ++p) {
                ...
                if (frame_idx >= ((uintptr_t)frame_bitmap / FRAME_SIZE) &&
                    frame_idx < (((uintptr_t)frame_bitmap / FRAME_SIZE) + bitmap_pages)) {
                    continue;
                }
