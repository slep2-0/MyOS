#!/usr/bin/env python3
"""Build and run the isolated user-mode loader runtime gate."""
...
def _build(configuration: str, jobs: int) -> Path:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"loader build failed with exit code {result.returncode}")
...
def _run_one(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"loader {cpu_count}-CPU run timed out after {timeout_seconds}s; "
    ...
    user_pass = "MT-LOADER USER PASS" in debug_text
    final_pass = "MT-STRESS PASS LOADER" in debug_text
    if result.returncode != PASS_EXIT_CODE or not user_pass or not final_pass:
        markers = [
            line for line in debug_text.splitlines()
            if line.startswith("MT-LOADER") or line.startswith("MT-STRESS")
        ]
        marker_text = markers[-1] if markers else "no loader marker"
        trace_text = f", and {qemu_trace}" if trace else ""
        raise RuntimeError(
            f"loader {cpu_count}-CPU run failed: QEMU exit={result.returncode}, "
...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_build:
            ...
            raise RuntimeError(f"loader image was not found at {image}")

        code, variables = _ovmf_paths()
        output_directory = BUILD_ROOT / "loader-test"
        ...
        for cpu_count in args.cpus:
            ...
            message = f"PASS loader {cpu_count} CPU(s) in {elapsed:.2f}s"
        ...
        print(f"LOADER TEST ERROR: {exc}", file=sys.stderr)
        return 1
