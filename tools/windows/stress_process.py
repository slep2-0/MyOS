#!/usr/bin/env python3
"""Build and run the isolated user-mode process creation gate."""
...
def _build(configuration: str, jobs: int) -> Path:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"process build failed with exit code {result.returncode}")
...
def _run_one(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            f"process {cpu_count}-CPU run timed out after {timeout_seconds}s; "
    ...
    user_pass = "MT-PROCESS USER PASS" in debug_text
    final_pass = "MT-STRESS PASS PROCESS" in debug_text
    if result.returncode != PASS_EXIT_CODE or not user_pass or not final_pass:
        markers = [
            line for line in debug_text.splitlines()
            if line.startswith("MT-PROCESS") or line.startswith("MT-STRESS")
        ]
        marker_text = markers[-1] if markers else "no process marker"
        raise RuntimeError(
            f"process {cpu_count}-CPU run failed: QEMU exit={result.returncode}, "
            f"last marker={marker_text}; see {debug_log} and {qemu_output}"
        )
    return elapsed
...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_build:
            ...
            raise RuntimeError(f"process image was not found at {image}")

        code, variables = _ovmf_paths()
        output_directory = BUILD_ROOT / "process-test"
        ...
        for cpu_count in args.cpus:
            ...
            message = f"PASS process {cpu_count} CPU(s) in {elapsed:.2f}s"
            with result_file.open("a", encoding="utf-8", newline="\n") as stream:
                stream.write(message + "\n")
            print(message, flush=True)
        return 0
    except (OSError, RuntimeError, subprocess.SubprocessError) as exc:
        print(f"PROCESS TEST ERROR: {exc}", file=sys.stderr)
        return 1
