#!/usr/bin/env python3
"""Build and run the isolated scheduler priority-preemption gate."""
...
def _build(configuration: str, jobs: int) -> Path:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"priority build failed with exit code {result.returncode}")
...
def _run_one(
    *,
    ...
    stem = f"priority_{cpu_count}cpu"
    ...
    try:
        ...
        raise RuntimeError(
            f"priority {cpu_count}-CPU run timed out after {timeout_seconds}s; "
    ...
    required = [
        "MT-SCHEDULER PRIORITY PASS",
        "MT-PRIORITY BLOCKED MUTATION PASS",
        "MT-PRIORITY SYNCHRONIZATION BOOST PASS",
        "MT-PRIORITY BOOST POLICY PASS",
        "MT-PRIORITY STARVATION BOUND PASS",
        "MT-PRIORITY READY MUTATION PASS",
        "MT-PRIORITY RUNNING MUTATION PASS",
        "MT-PRIORITY EQUAL ROTATION PASS",
        "MT-PRIORITY RUNTIME PASS",
        "MT-STRESS PASS PRIORITY",
    ]
    if cpu_count == 1:
        required.extend([
            "MT-PRIORITY STARVATION RUNTIME PASS",
            "MT-PRIORITY QUANTUM PRESERVE PASS",
            "MT-PRIORITY LOCAL PASS",
            "MT-PRIORITY REMOTE SKIP",
        ])
    else:
        required.extend([
            "MT-PRIORITY STARVATION RUNTIME SKIP",
            "MT-PRIORITY QUANTUM PRESERVE SKIP",
            "MT-PRIORITY LOCAL SKIP",
            "MT-PRIORITY REMOTE PASS",
        ])
    ...
    if result.returncode != PASS_EXIT_CODE or missing:
        markers = [
            ...
            if line.startswith("MT-PRIORITY") or
               line.startswith("MT-SCHEDULER") or
               line.startswith("MT-STRESS")
        ]
        marker_text = markers[-1] if markers else "no priority marker"
        raise RuntimeError(
            f"priority {cpu_count}-CPU run failed: QEMU exit={result.returncode}, "
...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_build:
            ...
            raise RuntimeError(f"priority image was not found at {image}")

        code, variables = _ovmf_paths()
        output_directory = BUILD_ROOT / "priority-test"
        ...
        for cpu_count in args.cpus:
            ...
            message = f"PASS priority {cpu_count} CPU(s) in {elapsed:.2f}s"
            with result_file.open("a", encoding="utf-8", newline="\n") as stream:
                stream.write(message + "\n")
            print(message, flush=True)
        return 0
    except (OSError, RuntimeError, subprocess.SubprocessError) as exc:
        print(f"PRIORITY TEST ERROR: {exc}", file=sys.stderr)
        return 1
