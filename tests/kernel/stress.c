...
#include "../../kernel/includes/exception.h"
#include "stress.h"
#include "ob_namespace.h"

#if MT_STRESS_MODE == MT_STRESS_MODE_HEAP
#include "../usermode/heap_test.h"
#endif

#if MT_STRESS_MODE == MT_STRESS_MODE_LOADER
#include "../usermode/loader_test.h"
#endif

#if MT_STRESS_MODE == MT_STRESS_MODE_TLS
#include "../usermode/tls_test.h"
#endif

#if MT_STRESS_MODE == MT_STRESS_MODE_PROCESS
#include "../usermode/process_test.h"
#endif

static const char StressEmptyProcessEnvironment[2] = { '\0', '\0' };
static const MT_CREATE_PROCESS_PARAMETERS StressTerminateMyselfProcessParameters = {
    ...
    .Environment = StressEmptyProcessEnvironment,
    .EnvironmentSize = sizeof(StressEmptyProcessEnvironment),
    .ParentProcess = 0,
...

{
    ...
    Stress5DRequireStatus(
        PsCreateProcess(
            &StressTerminateMyselfProcessParameters,
            &ProcessInformation,
            &InitialThread
        ),
        MT_SUCCESS,
...
)
{
    ...
    case MT_PROCESS_TEST_PRIORITY_EVENT:          return "PRIORITY-EVENT";
    case MT_PROCESS_TEST_PRIORITY_THREAD:         return "PRIORITY-THREAD";
    case MT_PROCESS_TEST_PRIORITY_SET:            return "PRIORITY-SET";
    case MT_PROCESS_TEST_PRIORITY_QUERY:          return "PRIORITY-QUERY";
    case MT_PROCESS_TEST_PRIORITY_NATIVE_CLASS:   return "PRIORITY-NATIVE-CLASS";
    case MT_PROCESS_TEST_PRIORITY_NATIVE_LENGTH:  return "PRIORITY-NATIVE-LENGTH";
    case MT_PROCESS_TEST_PRIORITY_NATIVE_POINTER: return "PRIORITY-NATIVE-POINTER";
    case MT_PROCESS_TEST_PRIORITY_NATIVE_VALUE:   return "PRIORITY-NATIVE-VALUE";
    case MT_PROCESS_TEST_PRIORITY_WORKER:         return "PRIORITY-WORKER";
    case MT_PROCESS_TEST_PRIORITY_CLOSE:          return "PRIORITY-CLOSE";
    default:                                     return "UNKNOWN-EXIT";
    }
}
...
#if MT_STRESS_MODE == MT_STRESS_MODE_BALANCING
#define STRESS_BALANCING_MAX_PROCESSORS 32U
#define STRESS_BALANCING_MAX_SYNTHETIC_THREADS 128U
#define STRESS_BALANCING_PREFERRED_LOAD_GAP 8U
#define STRESS_BALANCING_QUEUE_SCAN_LIMIT 4096U

typedef enum _STRESS_BALANCING_FAILURE {
    StressBalancingUnexpectedStatus = 1,
    StressBalancingProtocolTimeout,
    StressBalancingProcessorMismatch,
    StressBalancingSelectionMismatch,
    StressBalancingQueueOwnershipMismatch,
    StressBalancingSyntheticThreadLimit,
    StressBalancingReadyQueueCountMismatch,
    StressBalancingReadyQueueTopology
} STRESS_BALANCING_FAILURE;

typedef struct _STRESS_BALANCING_HOLDER_CONTEXT {
    EVENT Gate;
    volatile MTSTATUS WaitStatus;
    volatile uint32_t Processor;
    volatile bool Holding;
    volatile bool Release;
} STRESS_BALANCING_HOLDER_CONTEXT;

static ETHREAD StressBalancingSyntheticThreads[
    STRESS_BALANCING_MAX_SYNTHETIC_THREADS
];

NORETURN
static void
StressBalancingBugCheck(
    STRESS_BALANCING_FAILURE Failure,
    void* Parameter2,
    void* Parameter3
)
{
    StressAutomationWriteText("MT-BALANCING FAIL\n");
    MeBugCheckEx(
        SCHEDULER_FAILURE,
        (void*)(uintptr_t)(0xB000U + Failure),
        Parameter2,
        Parameter3,
        MeGetCurrentProcessor()
    );
...
)
{
    if (Actual != Expected) {
        StressBalancingBugCheck(
            StressBalancingUnexpectedStatus,
            Detail,
            (void*)(uintptr_t)Actual
        );
    }
...
static uint32_t
StressBalancingReadReadyCount(
    PPROCESSOR Processor
)
{
    IRQL OldIrql;
    MsAcquireSpinlock(&Processor->readyQueue.Lock, &OldIrql);
    uint32_t Count = Processor->readyQueue.ThreadCount;
    MsReleaseSpinlock(&Processor->readyQueue.Lock, OldIrql);
    return Count;
}

static void
StressBalancingValidateReadyQueue(
    PPROCESSOR Processor,
    void* Detail
)
{
    uint32_t ObservedCount = 0;
    bool TopologyValid = true;
    PREADY_QUEUE Queue = &Processor->readyQueue;
    ...
    while (TopologyValid && Entry != ListHead) {
        if (!Entry ||
            ...
            Entry->Blink != Previous ||
            Entry->Flink->Blink != Entry ||
            ObservedCount >= STRESS_BALANCING_QUEUE_SCAN_LIMIT) {
            TopologyValid = false;
            break;
        }
    ...
    uint32_t RecordedCount = Queue->ThreadCount;
    MsReleaseSpinlock(&Queue->Lock, OldIrql);
...
static void
StressBalancingHolderWorker(
    THREAD_PARAMETER Parameter
)
{
    STRESS_BALANCING_HOLDER_CONTEXT* Context = Parameter;
    MTSTATUS Status = MsWaitForSingleObject(
        &Context->Gate,
        KernelMode,
        false,
    ...
    InterlockedStoreRelease(&Context->WaitStatus, Status);
    if (Status != MT_SUCCESS) {
        InterlockedStoreRelease(&Context->Holding, true);
        return;
    }
...
static PETHREAD
StressBalancingQueueSyntheticThread(
    uint32_t AllowedMask,
    PPROCESSOR PreferredProcessor,
    uint32_t* SyntheticCount,
    void* Detail
)
{
    if (*SyntheticCount >= STRESS_BALANCING_MAX_SYNTHETIC_THREADS) {
        StressBalancingBugCheck(
            StressBalancingSyntheticThreadLimit,
            Detail,
            (void*)(uintptr_t)*SyntheticCount
        );
    }

    PETHREAD Thread =
        &StressBalancingSyntheticThreads[(*SyntheticCount)++];
    kmemset(Thread, 0, sizeof(*Thread));
    InitializeListHead(&Thread->SchedulerListEntry);
    Thread->InternalThread.SchedulerLock.locked = 0;
    Thread->InternalThread.BasePriority = MT_PRIORITY_NORMAL;
    Thread->InternalThread.Priority = MT_PRIORITY_NORMAL;
    Thread->InternalThread.TimeSlice = DEFAULT_TIMESLICE_TICKS;
    Thread->InternalThread.TimeSliceAllocated = DEFAULT_TIMESLICE_TICKS;
    Thread->InternalThread.ThreadState = THREAD_READY;
    Thread->InternalThread.AllowedProcessorMask = AllowedMask;

    if (!MeQueueThreadOnAllowedProcessor(Thread, PreferredProcessor)) {
        StressBalancingBugCheck(
            StressBalancingQueueOwnershipMismatch,
            Detail,
            Thread
    ...
    if (!Owner || (AllowedMask & (1u << Owner->ID)) == 0) {
        StressBalancingBugCheck(
            StressBalancingQueueOwnershipMismatch,
            Detail,
            Owner
...
static void
StressBalancingRemoveSyntheticThreads(
    uint32_t SyntheticCount
)
{
    while (SyntheticCount != 0) {
        PETHREAD Thread =
            &StressBalancingSyntheticThreads[--SyntheticCount];
        PPROCESSOR Owner = InterlockedLoadAcquire(
            &Thread->InternalThread.ReadyProcessor
        ...
        IRQL OldIrql;
        MsAcquireSpinlock(&Owner->readyQueue.Lock, &OldIrql);
        MsAcquireSpinlockAtDpcLevel(
            &Thread->InternalThread.SchedulerLock
        );

        bool Valid =
            InterlockedLoadAcquire(&Thread->InternalThread.ThreadState) ==
                THREAD_READY &&
            InterlockedLoadAcquire(&Thread->InternalThread.ActiveProcessor) ==
                NULL &&
            InterlockedLoadAcquire(&Thread->InternalThread.ReadyProcessor) ==
                Owner;
        bool Removed = Valid &&
            MeRemoveThreadFromQueue(&Owner->readyQueue, Thread);
        if (Removed) {
            InterlockedStoreRelease(
                &Thread->InternalThread.ReadyProcessor,
                NULL
        ...
        MsReleaseSpinlock(&Owner->readyQueue.Lock, OldIrql);
...
static void
StressBalancingTestSingleton(
    PPROCESSOR ControllerProcessor
)
{
    ETHREAD Synthetic = { 0 };
    InitializeListHead(&Synthetic.SchedulerListEntry);
    Synthetic.InternalThread.SchedulerLock.locked = 0;
    Synthetic.InternalThread.AllowedProcessorMask =
        1u << ControllerProcessor->ID;

    PPROCESSOR Selected = MepSelectLeastLoadedAllowedProcessor(
        &Synthetic.InternalThread,
        ControllerProcessor
    );
    if (Selected != ControllerProcessor) {
        StressBalancingBugCheck(
            StressBalancingSelectionMismatch,
            ControllerProcessor,
            Selected
    ...
    StressAutomationWriteText("MT-BALANCING SINGLETON PASS\n");
}

static void
StressBalancingTestSmpPlacement(
    PPROCESSOR ControllerProcessor
)
{
    uint32_t ProcessorCount = MeGetActiveProcessorCount();
    PPROCESSOR Targets[STRESS_BALANCING_MAX_PROCESSORS] = { 0 };
    PETHREAD Holders[STRESS_BALANCING_MAX_PROCESSORS] = { 0 };
    STRESS_BALANCING_HOLDER_CONTEXT Contexts[
        STRESS_BALANCING_MAX_PROCESSORS
    ] = { 0 };
    uint32_t TargetCount = 0;
    uint32_t AllowedMask = 0;

    for (uint32_t Index = 0; Index < ProcessorCount; Index++) {
        PPROCESSOR Processor = MeGetProcessorBlock((uint8_t)Index);
        if (Processor == ControllerProcessor ||
            InterlockedLoadAcquire(&Processor->State) !=
                ProcessorStateOnline) {
            continue;
        }

        Targets[TargetCount] = Processor;
        AllowedMask |= 1u << Processor->ID;
        MsInitializeEvent(
            &Contexts[TargetCount].Gate,
            DispatcherSynchronizationEvent,
            false
        );
        Contexts[TargetCount].WaitStatus = MT_PENDING;
        Contexts[TargetCount].Processor = UINT32_MAX;

        Holders[TargetCount] = Stress6CreateRetainedThread(
            StressBalancingHolderWorker,
            &Contexts[TargetCount]
        );
        Stress6WaitForDispatcherWait(
            Holders[TargetCount],
            &Contexts[TargetCount].Gate.Header,
            (void*)(uintptr_t)(0xB100U + TargetCount)
        );

        uint32_t PreviousMask = UINT32_MAX;
        StressBalancingRequireStatus(
            MeSetThreadAffinityMask(
                &Holders[TargetCount]->InternalThread,
                1u << Processor->ID,
                &PreviousMask
            ),
            MT_SUCCESS,
            (void*)(uintptr_t)(0xB120U + TargetCount)
        );
        StressBalancingRequireStatus(
            MsSetEvent(&Contexts[TargetCount].Gate),
            MT_SUCCESS,
            (void*)(uintptr_t)(0xB140U + TargetCount)
        );
        Stress6WaitForFlag(
            &Contexts[TargetCount].Holding,
            (void*)(uintptr_t)(0xB160U + TargetCount)
        );
        StressBalancingRequireStatus(
            InterlockedLoadAcquire(&Contexts[TargetCount].WaitStatus),
            MT_SUCCESS,
            (void*)(uintptr_t)(0xB180U + TargetCount)
        );
        if (InterlockedLoadAcquire(&Contexts[TargetCount].Processor) !=
                Processor->ID) {
            StressBalancingBugCheck(
                StressBalancingProcessorMismatch,
                (void*)(uintptr_t)Processor->ID,
                (void*)(uintptr_t)InterlockedLoadAcquire(
                    &Contexts[TargetCount].Processor
                )
            );
        }
    ...
    StressAutomationWriteText("MT-BALANCING HOLD PASS\n");

    uint32_t SyntheticCount = 0;
    IRQL TestOldIrql;
    MeRaiseIrql(DISPATCH_LEVEL, &TestOldIrql);

    uint32_t MaximumCount = 0;
    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        uint32_t Count = StressBalancingReadReadyCount(Targets[Index]);
    ...
    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        ...
        while (Count++ < MaximumCount) {
            PETHREAD Thread = StressBalancingQueueSyntheticThread(
                1u << Targets[Index]->ID,
                Targets[Index],
                &SyntheticCount,
                (void*)(uintptr_t)(0xB200U + SyntheticCount)
            );
            if (InterlockedLoadAcquire(
                    &Thread->InternalThread.ReadyProcessor
                ) != Targets[Index]) {
    ...
    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        if (StressBalancingReadReadyCount(Targets[Index]) != MaximumCount) {
            StressBalancingBugCheck(
                StressBalancingReadyQueueCountMismatch,
                Targets[Index],
                (void*)(uintptr_t)MaximumCount
            );
        }
    }

    PPROCESSOR PreferredProcessor = Targets[TargetCount - 1U];
    PETHREAD TieThread = StressBalancingQueueSyntheticThread(
        AllowedMask,
        PreferredProcessor,
        &SyntheticCount,
        (void*)0xB300
    );
    if (InterlockedLoadAcquire(&TieThread->InternalThread.ReadyProcessor) !=
            PreferredProcessor) {
        StressBalancingBugCheck(
            StressBalancingSelectionMismatch,
            PreferredProcessor,
            InterlockedLoadAcquire(
                &TieThread->InternalThread.ReadyProcessor
            )
        );
    }

    for (uint32_t Index = 0;
         Index < STRESS_BALANCING_PREFERRED_LOAD_GAP;
         Index++) {
        StressBalancingQueueSyntheticThread(
            1u << PreferredProcessor->ID,
            PreferredProcessor,
            &SyntheticCount,
            (void*)(uintptr_t)(0xB320U + Index)
        );
    }

    PPROCESSOR ExpectedLeast = NULL;
    uint32_t LeastCount = UINT32_MAX;
    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        uint32_t Count = StressBalancingReadReadyCount(Targets[Index]);
        if (Count < LeastCount) {
            LeastCount = Count;
            ExpectedLeast = Targets[Index];
        }
    }

    PETHREAD LeastThread = StressBalancingQueueSyntheticThread(
        AllowedMask,
        PreferredProcessor,
        &SyntheticCount,
        (void*)0xB340
    );
    if (InterlockedLoadAcquire(&LeastThread->InternalThread.ReadyProcessor) !=
            ExpectedLeast) {
        StressBalancingBugCheck(
            StressBalancingSelectionMismatch,
            ExpectedLeast,
            InterlockedLoadAcquire(
                &LeastThread->InternalThread.ReadyProcessor
            )
        );
    }

    PETHREAD AffinityThread = StressBalancingQueueSyntheticThread(
        1u << PreferredProcessor->ID,
        Targets[0],
        &SyntheticCount,
        (void*)0xB350
    ...
    if (InterlockedLoadAcquire(
            &AffinityThread->InternalThread.ReadyProcessor
        ) != PreferredProcessor) {
        StressBalancingBugCheck(
            StressBalancingSelectionMismatch,
            PreferredProcessor,
    ...
    StressAutomationWriteText("MT-BALANCING PLACEMENT PASS\n");

    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        StressBalancingValidateReadyQueue(
            Targets[Index],
            (void*)(uintptr_t)(0xB400U + Index)
        );
    }

    StressBalancingRemoveSyntheticThreads(SyntheticCount);

    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        StressBalancingValidateReadyQueue(
            Targets[Index],
            (void*)(uintptr_t)(0xB440U + Index)
        );
    }

    MeLowerIrql(TestOldIrql);
    StressAutomationWriteText("MT-BALANCING ACCOUNTING PASS\n");

    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        InterlockedStoreRelease(&Contexts[Index].Release, true);
    }
    for (uint32_t Index = 0; Index < TargetCount; Index++) {
        Stress6JoinThread(
            Holders[Index],
            (void*)(uintptr_t)(0xB480U + Index)
        );
    }
}

static void
StressBalancingController(void)
{
    gop_printf(
        COLOR_GREEN,
        "MT-BALANCING START (%u CPUs)\n",
        MeGetActiveProcessorCount()
    );
    ...
    StressBalancingTestSingleton(ControllerProcessor);
    if (MeGetActiveProcessorCount() == 1) {
        StressAutomationWriteText("MT-BALANCING SMP SKIP\n");
    }
    else {
        StressBalancingTestSmpPlacement(ControllerProcessor);
    }
    ...
    StressAutomationWriteText("MT-BALANCING RUNTIME PASS\n");
}
#endif
...
#elif MT_STRESS_MODE == MT_STRESS_MODE_BALANCING
    StressBalancingController();
    StressAutomationPass("BALANCING");
#elif MT_STRESS_MODE == MT_STRESS_MODE_NAMESPACE
    StressNamespaceController();
    StressAutomationPass("NAMESPACE");
#else
